...
// Recuperation de l'article avec le stock associe
$stmt = $conn->prepare("
...
$stock_qty     = (int)$article['stock_quantity'];
...
<div class="detail-container">
    ...
    <div class="detail-card">
        <div class="detail-image-wrap">
            ...
            <?php else: ?>
                <img src="uploads/default.png" alt="Image par defaut" class="detail-image">
            <?php endif; ?>
        </div>
        ...
        <div class="detail-info">
            ...
            <div class="detail-meta">
                ...
                <span>Publie le <?= htmlspecialchars(substr($article['published_at'], 0, 10)) ?></span>
            </div>

            <div class="detail-price"><?= number_format($article['price'], 2) ?> EUR</div>

            <!-- Indicateur de stock -->
            <div class="stock-badge <?= $out_of_stock ? 'stock-empty' : ($stock_qty <= 3 ? 'stock-low' : 'stock-ok') ?>">
                <?php if ($out_of_stock): ?>
                    Stock epuise
                <?php elseif ($stock_qty <= 3): ?>
                    Plus que <?= $stock_qty ?> en stock
                <?php else: ?>
                    <?= $stock_qty ?> disponibles
                <?php endif; ?>
            </div>
            ...
            <?php if (isset($_SESSION['user_id']) && $_SESSION['role'] !== 'guest'): ?>
                <?php if ($out_of_stock): ?>
                    <button class="btn-cart-detail" disabled style="opacity:0.45; cursor:not-allowed;">
                        Stock epuise
                    </button>
                <?php else: ?>
            ...
            <?php if ($is_author || $is_admin): ?>
                <div style="margin-top:1rem;">
                    <a href="edit?id=<?= (int)$article['id'] ?>" class="btn btn-secondary" style="font-size:0.82rem; padding:0.5rem 1.2rem;">Modifier l'article</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
.stock-badge { display:inline-block; padding:0.25rem 0.85rem; border-radius:50px; font-size:0.78rem; font-weight:600; }
.stock-ok   { background:rgba(46,122,46,0.1); color:#2d6a2d; }
.stock-low  { background:rgba(180,110,20,0.1); color:#8a5a10; }
.stock-empty{ background:rgba(140,46,46,0.1); color:#7a2a2a; }
...
.btn-cart-detail:hover:not([disabled]) { background:var(--accent); transform:translateY(-1px); color:#fff; }
